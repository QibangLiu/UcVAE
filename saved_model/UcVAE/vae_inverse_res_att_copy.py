...
# %%
current_filename='vae_inverse_res_att'
filebase='./saved_model/VAE_res_att/'
trainmodel=False
...
def EncoderModel(n_input=4, latent_dim=2):
    inputs = keras.Input(shape=(n_input,), name='input_layer')
    ...
    model = tf.keras.Model(inputs, [mean, log_var, latent], name="Encoder")
    return model
...
class VAE(keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            z_mean, z_log_var, z = self.encoder(x_inp)
...
if os.path.isfile(filename_encoder):
   ...
   encoder = EncoderModel(nX,latent_dim)
...
if os.path.isfile(filename_decoder):
    decoder=tf.keras.models.load_model(filename_decoder,compile=False)
else:
...
test_validate={'Yplot_target':Yplot_target[ids],'Yplot_design_uvae':Yplot_design[ids]}
...
def design_ontest(id,scramble=True):
    ...
    sc=ax.scatter(x_design[:,0],x_design[:,1]*100,c=NRMSE_diff, cmap='summer',s=10)
    ax.scatter(Xtarg[:,0],Xtarg[:,1]*100,c='r', marker='*',s=20,label='target')
    ax.scatter(x_design[id_sort[0],0],x_design[id_sort[0],1]*100,c='b', marker='^',s=20,label='best')
    ax.legend()
    ax=plt.subplot(1,2,2)
    ax.set_ylabel(r'$d$')
    ax.set_xlabel(r'$h_L~[\frac{W}{K m^2}]$')
    sc=ax.scatter(x_design[:,2],x_design[:,3],c=NRMSE_diff, cmap='summer',s=10)
    ax.scatter(Xtarg[:,2],Xtarg[:,3],c='r', marker='*',s=20,label='target')
    ax.scatter(x_design[id_sort[0],2],x_design[id_sort[0],3],c='b', marker='^',s=20,label='best')
    ax.legend()
...
def design_ondemand(img,scramble=False):
    ...
    id_sort=np.argsort(NRMSE_diff)
    fid+=1
    fig=plt.figure(num=fid,figsize=(6.4,3))
    ...
    idx=id_sort[int(id_sort.shape[0]*0.5)]
    ax.imshow(YY_de[idx])
    ax.axis('scaled')
    ax.axis('off')
    ax.set_title(r'NRMSE=%.2f'%NRMSE_diff[idx])
    ...
    cbar=plt.colorbar(sc)
    cbar.set_label('NRMSE')
    cbar.locator = ticker.MultipleLocator(0.02) 
    cbar.update_ticks()  
    return x_design[id_sort[0]]
...
x_design=design_ondemand(img,scramble=True)
print(x_design)
